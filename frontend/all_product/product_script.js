...
function searchFunction() {
  var input, filter, ul, li, a, i, txtValue, source;
  ...
  for (i = 0; i < li.length; i++) {
    ...
    source = txtValue.toUpperCase();
    if(filter === source || source.includes(filter)) {
        window.location.href = a.getAttribute("href"); // Navigate to the target ID
        break; // Exit the loop after finding the first match
        
    } 
    if(i == li.length - 1) {
        alert("Sorry, we don't have that book");
